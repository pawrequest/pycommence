...
class PyCmcDDEServer(DDEServer):
    ...
    def view_reset(self, category: str):
        self.send_message(msgs.view.view_category(category))
    ...
    def item_read(self, category, name, fields: list[str] = None) -> dict[str, str]:
        ...
        master_msg = msgs.get.get_fields(category=category, item=name, fields=field_names, delim=DELIM)
        ...
        if len(str(master_msg)) < self.options.max_cmd_len:
            ...
            for start in range(0, len(field_names), self.options.fields_chunk):
                ...
                chunk_msg = msgs.get.get_fields(category=category, item=name, fields=fields_chunk, delim=DELIM)
    ...
    def item_edit(
        ...
        for field_name, field_value in field_updates.items():
            msg = msgs.execute.execute_edit_item(category, item_name, field_name, field_value, topic)
