...
from pycommence.dde.types import DDETopic, DDEExecute
...
def execute_add_item(
    ...
    msg = DDEExecute(func_name='AddItem', params=params)
    msg.topic = topic
    return msg
...
def execute_show_item(category: str, item: str, form_name: str | None = None) -> DDEExecute:
    return DDEExecute(func_name='ShowItem', params=[category, item, form_name])


def execute_show_view(view_name: str, force_new_copy: int | None = None) -> DDEExecute:
    """
...
def execute_merge_template_create(name: str, category: str, shared: bool | int) -> DDEExecute:
    """
...
def execute_merge_template_save(name: str, shared: bool | int) -> DDEExecute:
    shared_val = int(shared) if isinstance(shared, bool) else shared
