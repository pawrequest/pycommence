...
def dde_show_item(category: str, item: str, form_name: str | None = None) -> str:
    return _dde_format_function('ShowItem', [category, item, form_name])


def dde_show_view(view_name: str, force_new_copy: int | None = None) -> str:
    """
...
def dde_get_view_to_file(view_name: str, mode: int, param1: str | None, param2: str | None, filename: str) -> str:
...
def dde_merge_template_create(name: str, category: str, shared: bool | int) -> str:
    """
...
def dde_merge_template_save(name: str, shared: bool | int) -> str:
    shared_val = int(shared) if isinstance(shared, bool) else shared
