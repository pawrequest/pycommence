...
def get_csr(cmc: auto_cmc.ICommenceDB, table_name) -> auto_cmc.ICommenceCursor:
    return cmc.GetCursor(0, table_name, 0)


def qs_from_name(
    cmc: auto_cmc.ICommenceDB, table, record, edit=False
) -> auto_cmc.ICommenceQueryRowSet | auto_cmc.ICommenceEditRowSet:
    ...
    if results.GetRowValue(0, 0, 0) != record:
        raise ValueError(
            f'Expected {record} but got ' f'{results.GetRowValue(0, 0, 0)}'
        )
    return results


def connected_records_to_qs(
    cmc: auto_cmc.ICommenceDB, connect: CONNECTION, item_name: str, max_res=50
) -> auto_cmc.ICommenceQueryRowSet | None:
    ...
    if max_res and qs.RowCount > max_res:
        raise ValueError(
            f'Query set has {qs.RowCount} rows, ' f'when {max_res} was set as max'
        )
    return qs
...
def filter_by_date(
    cursor: auto_cmc.ICommenceCursor,
    field_name: str,
    date: datetime.date,
    condition='After',
):
...
def filter_by_field_new(
    cursor: auto_cmc.ICommenceCursor, field_name: str, condition, value=None
):
...
def filter_by_field_old(
    cursor: auto_cmc.ICommenceCursor, field_name: str, value, contains=False
):
    rationale = 'Contains' if contains else 'Equal To'
    filter_str = f'[ViewFilter(1, F,, "{field_name}", "{rationale}", "{value}")]'  # noqa: E231
    res = cursor.SetFilter(filter_str, 0)
    if not res:
...
def filter_by_connection(
    cursor: auto_cmc.ICommenceCursor, item_name: str, connection: CONNECTION
):
    filter_str = (
        f'[ViewFilter(1, CTI,, {connection.value.desc}, '  # noqa: E231
        f'{connection.value.value_table}, {item_name})]'
    )
    res = cursor.SetFilter(filter_str, 0)
    if not res:
        raise ValueError(
            f'Could not set filter for ' f'{connection.value.desc} = {item_name}'
        )
...
def qs_to_lists(qs, max_rows=None) -> List:
    ...
    if max_rows and qs.RowCount > max_rows:
        raise ValueError(
            f'Query set has {qs.RowCount} rows, ' f'more than {max_rows} rows requested'
        )
    rows = []
    delim = '%^&£$_+'
...
def qs_to_dicts(qs: auto_cmc.ICommenceQueryRowSet, max_rows=None) -> List[dict]:
    ...
    if max_rows and row_count > max_rows:
        raise ValueError(
            f'Query set has {row_count} rows, ' f'more than {max_rows} rows requested'
        )
